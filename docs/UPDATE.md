...
## Обновление до v2.5.0

Замените файлы из архива и обязательно обновите `nginx/default.conf`, поскольку защита отчета при отключенном публичном доступе реализована через Nginx `auth_request`.
...
После запуска в SQLite автоматически создается таблица `app_users`, а параметр `public_report_enabled` получает значение `1`. Поэтому сразу после обновления существующий публичный просмотр отчета сохраняется.

Сначала войдите встроенной учетной записью администратора из `.env`, откройте «Настройки» → «Пользователи и права» и создайте необходимые записи.
