...
def export_filename(
    report: TestExportReport,
    extension: str,
) -> str:
    safe_name = _safe_test_name(
        report.test_name
    )

    timestamp = report.created_at.strftime(
        "%Y%m%d%H%M"
    )
...
def _metadata_lines(
    ...
    return (
        ...
        (
            ...
            f"{report.created_at:%d.%m.%Y %H:%M}"
        ),
        (
            f"Участников: {report.participants} | "
            f"Пройдено: {report.completed} | "
            f"Не прошли: {report.failed} | "
            f"Ожидают прохождения: {report.waiting}"
        ),
    )
...
def build_xlsx(
    ...
    title_text, test_text, date_text, stats_text = (
    ...
    worksheet["A2"] = test_text
    worksheet["A3"] = date_text
    worksheet["A4"] = stats_text
    ...
    for cell_name in (
        ...
        worksheet[cell_name].alignment = Alignment(
            horizontal="left",
            vertical="center",
        )
